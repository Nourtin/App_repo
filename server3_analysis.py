...
def render_server2_section(server2_files: list):
    ...
    with tab_stat:
        ...
        with col1:
            ...
            fig = px.pie(df_cat, names="Catégorie", values="Appels",
                         title="Par catégorie", color_discrete_sequence=px.colors.qualitative.Set3, hole=0.35)
        ...
        with col2:
            ...
            fig2 = px.bar(df_stat, x="Appels", y="Statut", orientation="h",
                          title="Top 15 statuts bruts", color="Appels", color_continuous_scale="Blues")
        ...
        df_recap = (df.groupby("categorie_statut")
                    .agg(Appels=("status", "count"), Durée_moy_sec=("length_in_sec", "mean"), Durée_tot_sec=("length_in_sec", "sum"))
                    .reset_index().sort_values("Appels", ascending=False))
        ...
        st.dataframe(df_recap[["categorie_statut", "Appels", "Part %", "Durée moy.", "Durée tot."]].rename(
            columns={"categorie_statut": "Catégorie"}), hide_index=True)  # ← corrigé

        if "length_in_sec" in df.columns:
            st.markdown("#### Durée moyenne par statut (top 10)")
            df_dur = (df.groupby("status")["length_in_sec"].agg(["mean", "count"]).reset_index()
                      .rename(columns={"mean": "Durée moy (s)", "count": "Appels"})
                      .query("Appels >= 5").sort_values("Durée moy (s)", ascending=False).head(10))
            fig3 = px.bar(df_dur, x="status", y="Durée moy (s)", title="Durée moyenne par statut (min. 5 appels)",
                          color="Durée moy (s)", color_continuous_scale="Oranges", text="Durée moy (s)")
    ...
    with tab_agent:
        ...
        df_agent = (df.groupby("user")
                    .agg(Appels=("status", "count"), Succès=("categorie_statut", lambda x: (x == "✅ Succès").sum()),
                         Durée_moy=("length_in_sec", "mean"), Durée_tot=("length_in_sec", "sum"))
                    .reset_index())
        ...
        col1, col2 = st.columns(2)
        with col1:
            fig = px.bar(df_agent.head(15), x="user", y="Appels", title="Volume d'appels par agent (top 15)",
                         color="Appels", color_continuous_scale="Blues", text="Appels")
            fig.update_traces(textposition="outside")
            fig.update_layout(showlegend=False, xaxis_title="Agent")
            st.plotly_chart(fig, use_container_width=True)
        with col2:
            fig2 = px.bar(df_agent.head(15), x="user", y="Taux succès %", title="Taux de succès par agent (top 15)",
                          color="Taux succès %", color_continuous_scale="Greens", text="Taux succès %")
        ...
        st.dataframe(df_agent[["user", "Appels", "Succès", "Taux succès %", "Durée moy.", "Durée tot."]].rename(
            columns={"user": "Agent"}),hide_index=True)  # ← corrigé

        if len(df_agent) >= 3:
            st.markdown("#### Volume vs Taux de succès")
            fig3 = px.scatter(df_agent, x="Appels", y="Taux succès %", text="user", size="Appels",
                              title="Positionnement des agents", color="Taux succès %", color_continuous_scale="RdYlGn")
    ...
    with tab_time:
        ...
        if "heure" not in df.columns:
            ...
            with col1:
                ...
                fig.add_trace(go.Bar(x=df_heure["heure"], y=df_heure["Appels"], name="Appels", marker_color="steelblue"), secondary_y=False)
                fig.add_trace(go.Scatter(x=df_heure["heure"], y=df_heure["Taux %"], name="Taux succès %",
                                         mode="lines+markers", line=dict(color="green", width=2)), secondary_y=True)
            ...
            with col2:
                if "date" in df.columns:
                    ...
                    fig2 = px.line(df_date, x="date", y=["Appels", "Succès"], title="Évolution journalière", markers=True)
            ...
            if "jour_sem" in df.columns:
                ...
                df_heat = (df.groupby(["jour_sem", "heure"])["status"].count().reset_index()
                ...
                fig3 = px.imshow(df_pivot, labels=dict(x="Heure", y="Jour", color="Appels"),
                                 title="Densité d'appels par heure et jour", color_continuous_scale="YlOrRd", aspect="auto")
                st.plotly_chart(fig3, use_container_width=True)  # ← corrigé

            if not df_heure.empty:
                best_hour = df_heure.loc[df_heure["Taux %"].idxmax()]
                st.success(f"🏆 Meilleure heure : **{int(best_hour['heure'])}h** — Taux succès {best_hour['Taux %']:.1f}% ({int(best_hour['Appels'])} appels)")
    ...
    with tab_leads:
        ...
        if "lead_id" not in df.columns:
            ...
            fig = px.bar(dist, x="Nb appels", y="Nb leads",
                         title="Distribution : combien de fois un lead est appelé ?",
                         color="Nb leads", color_continuous_scale="Purples", text="Nb leads")
            fig.update_traces(textposition="outside")
            ...
            st.dataframe(top_leads, use_container_width=True, hide_index=True)  # ← corrigé
            ...
            if "categorie_statut" in df.columns:
                ...
                st.metric("Leads recyclables (dernier statut = NA/Occupé/Répondeur)",
                          f"{len(recyclable):,}", delta=f"{round(len(recyclable)/len(df_last)*100, 1)}% du total")
